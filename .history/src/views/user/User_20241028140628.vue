...
export default {
  ...
  created() {
    ...
    axios
      .get(
        this.$store.state.globalhost + "user/list/?page=1&size=" + this.pagesize
      )
      .then((resp) => {
        ...
        if (valid) {
          this.$confirm("确定添加该用户信息?", "提示", {
            ...
            .then(() => {
              ...
              axios
                .post(
                  this.$store.state.globalhost + "user/addUser",
                  _this.userform
                )
                .then(function (resp) {
                  if (resp.data) {
    ...
    editSubmit() {
      this.$confirm("确定修改该用户信息?", "提示", {
        ...
        .then(() => {
          ...
          axios
            .put(
              this.$store.state.globalhost + "user/updateUser",
              _this.editform
            )
            .then(function (resp) {
              if (resp.data) {
    ...
    deleteUser(row) {
      this.$confirm("确定删除该用户?", "提示", {
        ...
        .then(() => {
          let _this = this;
          axios
            .delete(this.$store.state.globalhost + "user/deleteUser/" + row.id)
            .then(function (resp) {
              if (resp.data) {
                _this.$message({
                  type: "success",
